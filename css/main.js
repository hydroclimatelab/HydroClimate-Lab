...
// ── GALLERY ────────────────────────────────────────────
  ...
  if (gg && d.gallery) {
    gg.innerHTML = '';
    // Main page par sirf 3 photos dikhane ke liye slice kiya
    d.gallery.slice(0, 3).forEach(g => {
    ...
    // Main page ke grid ke niche "View Full Gallery" ka button lagana (Aapke original team-btn ke style mein)
    const viewAllDiv = document.createElement('div');
    viewAllDiv.style.marginTop = '36px';
    viewAllDiv.innerHTML = `<a href="gallery.html" class="btn-primary">View Full Gallery →</a>`;
    gg.after(viewAllDiv);
  }

  const lbCloseBtn = document.getElementById('lb-close');
  if (lbCloseBtn && lb) {
    lbCloseBtn.addEventListener('click', () => lb.classList.remove('open'));
